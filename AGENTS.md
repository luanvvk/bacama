...
- **Auth:** [Clerk](https://clerk.com) is wired up (Phase 3, `docs/BUILD-PLAN.md` §3.1–3.4) — `@clerk/nextjs`, `<ClerkProvider>` in `src/app/layout.tsx`, `clerkMiddleware` in `src/proxy.ts`, and a `src/app/api/webhooks/clerk/route.ts` that upserts the `User` row. `/login` and `/register` are real Clerk flows built on our usual `react-hook-form` + zod + `Controlled*` stack (not Clerk's prebuilt components). Route protection is `src/lib/auth/guards.ts` (`requireUser`/`requireRoles`), called from a layout/page — `getCurrentUser` in `src/lib/providers/auth/clerk.ts` is wrapped in React's `cache()` so calling the guard from both a layout and its page dedupes to one lookup per request. `src/components/auth/GuestGate` is now only for genuinely-not-signed-in or not-yet-built states (e.g. `/teach`), not a stand-in for auth. **Still missing:** `CLERK_SECRET_KEY` / `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` / webhook signing secret in `.env` (B3-a) — the app won't run without them.
...
- **External providers (payment, shipping, video, email, auth):** each sits behind a small interface in `src/lib/providers/<concern>/types.ts` (payment, courier, local-handoff, video, email, auth), with per-vendor adapters and a factory (`index.ts`) selecting the active one. Pages/route handlers call the interface — `getPaymentProvider(...).pay(...)` — never a vendor SDK directly. **Reviewer rule:** no vendor SDK imported and no vendor API called outside that vendor's own adapter file, or the PR doesn't merge. (Vendor names as display text — a payment tile reading "ZaloPay" — or as stored data are fine; the rule is about coupling. See `docs/BUILD-PLAN.md` §3.1 for the exact check.) All six factories currently throw `not implemented` — real adapters land in later phases as each provider is wired up.
