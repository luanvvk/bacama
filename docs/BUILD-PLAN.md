...
| Q15 | **`ProductCategory.bakery` overlaps `BakeryItem`.** The mock constants treat croissants as `Product`s; the PRD models them as per-site `BakeryItem`s with `bakesAt`/`sellOutBy`/`handoff`. The seed followed the PRD (and now has 7 real pastries seeded as `BakeryItem`, D11), so the `bakery` enum value is **unused** and has real data reinforcing the split. | Leaning toward dropping `bakery` from `ProductCategory` and having `/shop` merge `Product` + `BakeryItem` as two sources. **Not yet decided** — still needs the owner's sign-off, not assumed from the seed choice alone. | Phase 1, task 1.4                                            |
