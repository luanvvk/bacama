...
| Data layer    | Neon Postgres, **migrated and seeded with real business data** — 21 tables, 3 sites (1 real + 2 fictional-roadmap), 4 products, 7 bakery items, 8 menu items, 4 courses / 9 lessons. `prisma/schema.prisma`, `prisma/seed.ts`, `src/lib/prisma.ts` | Neon (`ap-southeast-1`)               |
...
| D11 | **The real GrabFood listing is the flagship site's ground truth; the other two sites stay fictional.** Seeded 2026-08-17 from `food.grab.com/.../bacama-coffee-more-delivery/5-C3KEGFM1VGN2N2`: real address (`K154/6 Lý Tự Trọng, Hải Châu, Đà Nẵng`, not any of the three original fictional addresses), real hours, a representative subset of the real 92-item/11-category menu across `Product` (4 real coffee bags, sold by bean type), `BakeryItem` (7 of the real ~50 pastries), and `MenuItem` (8 of the real ~35 café drinks, newly seeded — this model had no seed data before). The fictional single-origin coffee products (Đà Lạt Washed, Sơn La Natural, House Blend, Three Origins Box) were retired rather than kept alongside real ones — one `Product` catalog can't have two conflicting sources of truth. | Real data is more authoritative than invented PRD content once it exists, and duplicating a concept (packaged coffee) under two irreconcilable stories is exactly the two-sources-of-truth problem this project has flagged elsewhere. | If the fictional roastery-with-named-origins narrative turns out to matter more than the real menu (e.g. it's aspirational rebranding, not description), the real products should be reconsidered instead of just added alongside. |
...
- ~~**B0-b**~~ **superseded by D11** — seeded 2026-08-17 with the real flagship site + real menu subset from the live GrabFood listing (real Vietnamese copy, no translation review needed for that content), plus the two still-fictional roadmap sites from the mockup.
- **B0-c (owner):** Vietnamese copy for the parts that are still invented — the two fictional sites' text, and all course module/lesson titles (`coffee-shop-ui.html` doesn't cover courses) — **needs a native-speaker pass** before launch. The real GrabFood-sourced product/bakery/menu copy (D11) is the business's own and doesn't need this pass.
...
| Q15 | **`ProductCategory.bakery` overlaps `BakeryItem`.** The mock constants treated croissants as `Product`s; the PRD models them as per-site `BakeryItem`s with `bakesAt`/`sellOutBy`/`handoff`. The seed followed the PRD (and now has 7 real pastries seeded as `BakeryItem`, D11), so the `bakery` enum value was unused and reinforced the split. | **Resolved 2026-08-17 with owner approval:** drop `bakery` from `ProductCategory`. Keep `Product` and `BakeryItem` as separate database models; merge them only through a storefront catalog view/DTO. Bakery items remain handoff-only and must not enter the nationwide bean checkout. | done                                                         |
